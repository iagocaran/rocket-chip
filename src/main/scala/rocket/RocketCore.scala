...
case class RocketCoreParams(
  ...
  override val vpu: Option[VPUParams] = Some(VPUParams()),
  override val vLen: Int = 64,
  override val vMemDataBits: Int = 64
) extends CoreParams {
  val lgPauseCycles = 5
...
class Rocket(tile: RocketTile)(implicit p: Parameters) extends CoreModule()(p)
  ...
  when (mem_pc_valid) {
    ...
    wb_reg_wdata := Mux(
      io.vpu.wb_data.valid,
      io.vpu.wb_data.bits,
      Mux(
        mem_scie_pipelined, 
        mem_scie_pipelined_wdata,
        Mux(!mem_reg_xcpt && mem_ctrl.fp && mem_ctrl.wxd, 
          io.fpu.toint_data, 
          mem_int_wdata))
    )
  ...
  csr.io.vector.get.set_vconfig := Reg(Valid(new VConfig))
  ...
  val vpu_stall = io.vpu.stall_pipeline
