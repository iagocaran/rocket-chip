    ...
    with HasICacheFrontend
{
  ...
  nDCachePorts += 1 /*core */ + (dtim_adapter.isDefined).toInt + (tileParams.core.vpu.isDefined).toInt
...
class RocketTileModuleImp(outer: RocketTile) extends BaseTileModuleImp(outer)
  ...
  core.io.vpu <> vpuOpt.io
  dcachePorts += vpuOpt.io.mem
...
trait HasVpuModule { this: RocketTileModuleImp =>
  // TODO: Set vlen and elen from global values
  val vpuOpt = Module(new VPU(outer.tileParams.core.vpu.get)(outer.p))
}
